...
const SECTIONS = [
  ...
  ["models", "Model Usage"], ["sessions", "Sessions"],
];
...
// Token-usage-over-time granularity (also outside `state` so it survives refresh).
let trendMode = "1D";
const TREND = {
  session: { label: "Session", last: 30 },
  "1H": { size: 3600000, last: 48 },
  "1D": { size: 86400000, last: 30 },
  "7D": { size: 604800000, last: 12 },
  "30D": { size: 2592000000, last: 12 },
};
const trendToggle = () => `<span class="ltoggle">${["session", "1H", "1D", "7D", "30D"]
  .map((m) => `<span class="lt ${trendMode === m ? "on" : ""}" data-tm="${m}">${m === "session" ? "Session" : m}</span>`).join("")}</span>`;

// Returns token counts as a series for the current trendMode (gaps filled with 0).
function trendSeries(s) {
  if (trendMode === "session") {
    return (s.sessions || [])
      .map((x) => [Date.parse(x.start), x.totalTokens ?? 0]).filter((p) => !isNaN(p[0]))
      .sort((a, b) => a[0] - b[0]).slice(-TREND.session.last).map((p) => p[1]);
  }
  const { size, last } = TREND[trendMode];
  const buckets = new Map();
  for (const [t, tokens] of (s.tokenByHour || [])) {
    const k = Math.floor(t / size) * size;
    buckets.set(k, (buckets.get(k) || 0) + tokens);
  }
  const keys = [...buckets.keys()].sort((a, b) => a - b);
  if (!keys.length) { return []; }
  const out = [];
  for (let k = keys[0]; k <= keys[keys.length - 1]; k += size) { out.push(buckets.get(k) || 0); }
...
const charts = {
  ...
  spark(vals) {
    if (!vals.length) { return `<div class="hint">No data for this range yet.</div>`; }
    const w = 600, h = 90, max = Math.max(1, ...vals);
    const step = vals.length > 1 ? w / (vals.length - 1) : 0;
    const pts = vals.map((v, i) => `${(i*step).toFixed(1)},${(h - v/max*(h-8) - 4).toFixed(1)}`).join(" ");
...
function render() {
  ...
  document.querySelectorAll("[data-tm]").forEach((el) =>
    el.addEventListener("click", () => { trendMode = el.dataset.tm; render(); }));
...
function renderSection() {
  ...
  switch (state.section) {
    ...
    case "projects":
      if (!s) { return empty(); }
      return `<div class="panel"><h3>Project usage ${linesToggle()}</h3><table><tr><th>Project</th><th>Tokens</th><th>Cost</th><th>Lines</th><th>Last active</th></tr>
    ...
    case "models":
      if (!s) { return empty(); }
      return `<div class="panel"><h3>Model usage ${linesToggle()}</h3><table><tr><th>Model</th><th>Tokens</th><th>Cost</th><th>Lines</th></tr>
    ...
    default: // overview
      ...
      return `<div class="cards">
        ...
        <div class="panel"><h3>Token usage over time ${trendToggle()}</h3>${(() => {
            const series = trendSeries(s);
            const peak = series.length ? Math.max(...series) : 0;
            return charts.spark(series) + `<div class="hint">${trendMode === "session" ? "Per session" : "Per " + trendMode} · ${series.length} point${series.length === 1 ? "" : "s"} · peak ${tok(peak)} tokens</div>`;
          })()}</div>
